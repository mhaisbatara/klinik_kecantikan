...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const cValidation = await validatePayload(
      {
        kode: Joi.array().items(Joi.string()).min(1).required().label("Kode Shift"),
      },
      {
    ...
    await DB.transaction(async (trx) => {
      recordsBeforeDelete = await trx("mst_shift")
