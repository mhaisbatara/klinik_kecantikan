...
router.post("/", async (req, res) => {
  ...
  try {
    const cValidation = await validatePayload(
      { kode_jadwal: Joi.array().items(Joi.string()).min(1).required().label("Kode Jadwal") },
    ...
    return res.status(200).json({ status: status.SUKSES, message: "Jadwal karyawan berhasil dihapus", datetime: formatDateSystem() });
  } catch (error) {
    ...
    Logging(error, { file: "/master/jadwal_karyawan/jadwal_karyawan_delete.js", func: "delete", request: oPayload, response: oResult, user: username });
